...
class Add_payments_maintenance_controller extends GetxController{


  GlobalKey<FormState> formstate= GlobalKey<FormState>();
  late String name;
  late int maintenanceRequests_id;
  late String price;
  late String details;
  Add_payments_maintenance_services services =Add_payments_maintenance_services(Get.find());
  ...
  add_payments_maintenance_request() async{
    ...
    update();
    var response =await services.Add_payment_maintenance_request(maintenanceRequests_id,name,price,details);

    statusRequest=handlingdata(response);

    if(StatusRequest.succes==statusRequest){

      await Get.snackbar("تم الإضافة بنجاح", "تمت عملية إضافة واردات طلب الصيانة بنجاح",);

    }else if(StatusRequest.failure==statusRequest){
      await Get.snackbar("تنبيه", "لم يتم إضافة واردات طلب الصيانة",);

    }
    else{
      Get.defaultDialog(title: "حدث خطأ ما",content:Text("لم يتم إضافة واردات  طلب الصيانة"));
  ...
  @override
  void onInit() async{
    maintenanceRequests_id=Get.arguments['maintenanceRequests_id'];
    super.onInit();
  }
}