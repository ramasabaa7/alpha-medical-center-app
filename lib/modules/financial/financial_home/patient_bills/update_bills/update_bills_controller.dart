...
import 'package:project_after_update/modules/financial/financial_home/patient_bills/update_bills/update_bills_services.dart';
...
class Update_bills_controller extends GetxController {
  Update_bills_services services =
  Update_bills_services(Get.find(), Get.find(),Get.find());
  ...
  update_bill() async {
    ...
    statusRequest=StatusRequest.loading;
    update();
    var response =await services.update_bill(bill_id,id_visits,insurance_comp_id,discount_type,discount_details);

    statusRequest=handlingdata(response);

    if(StatusRequest.succes==statusRequest){

      await Get.snackbar("تمت الإضافة بنجاح", "تمت عملية تعديل الفاتورة بنجاح",);
      ...
      await Get.snackbar("تنبيه", "لم يتم تعديل  الفاتورة بنجاح",);
    }
    else{
      Get.defaultDialog(title: "حدث خطأ ما",content:Text("لم يتم تعديل  الفاتورة بنجاح "));
    }
    update();
  }
  ...
  void onInit() {
    ...
    id_visits=Get.arguments['Visit_ID'];
